...
// const table = require('../util/tableStorage');

const lib = new builder.Library('demo1');

lib.dialog('/email', [
  ((session, results, next) => {
    builder.Prompts.text(session, 'Please confirm your email address to get started')
  }),
  ((session, results, next) => {
    if (validator.isEmail(results.response)) {
      session.userData.email = results.response;
      session.send('Demo commencing.....')
...
//     table.startScriptOnlyOnRecognizedEmail(session, ifExists = () => {
//       session.send(`Great! Your email was recognized. Commencing demo.`)
//       session.beginDialog('/firstDialog')
//     }, ifNotExists = () => {
//       session.send('Sorry, this email was not recognized. Please contact your administrator')
//       session.endDialog()
//     })
//   })
// ]);
